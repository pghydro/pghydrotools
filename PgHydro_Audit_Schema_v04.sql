...
--
---------------------------------------------------------------------------------
--PgHYDRO Audit Scheme version 4.0 of 10/03/2016
---------------------------------------------------------------------------------
...
CREATE OR REPLACE FUNCTION pghydro.pghfn_audit_drainage_line() RETURNS TRIGGER AS $pghtb_audit_drainage_line$
    BEGIN
        ...
        IF (TG_OP = 'DELETE') THEN
            INSERT INTO pghydro.pghtb_audit_drainage_line SELECT 'D', now(), user, OLD.drn_pk, OLD.drn_gm;
            RETURN OLD;
        ELSIF (TG_OP = 'UPDATE') THEN
            INSERT INTO pghydro.pghtb_audit_drainage_line SELECT 'U', now(), user, NEW.drn_pk, NEW.drn_gm;
            RETURN NEW;
        ELSIF (TG_OP = 'INSERT') THEN
            INSERT INTO pghydro.pghtb_audit_drainage_line SELECT 'I', now(), user, NEW.drn_pk, NEW.drn_gm;
            RETURN NEW;
        END IF;
...
SELECT ads_drn_pk as drn_pk, ads_stamp, ads_userid, ads_drn_gm as drn_gm
